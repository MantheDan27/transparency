...
LRESULT CALLBACK MainWindow::WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_MAP_DEVICE_CLICK: {
        // Select and show the device in the Devices tab
        int devIdx = (int)wp;
        if (self->_tabDevices)
            self->_tabDevices->SelectAndShowDevice(devIdx);
        return 0;
    }
...
LRESULT MainWindow::OnCreate(HWND hwnd, LPCREATESTRUCT) {
    ...
    AddLedgerEntry(L"App Started", L"Transparencii v4.35.3 initialized");
