...
## 2025-06-20 - N+1 Execution Bottleneck in Bulk Updates
**Learning:** For Firestore bulk operations in frontend clients (e.g., uploading or syncing devices), replacing sequential `await setDoc()` calls inside loops with an array of promises executed via `Promise.all()` significantly resolves N+1 execution bottlenecks.
**Action:** When performing bulk writes to Firestore, always collect the promises and wait for them concurrently via `Promise.all` rather than awaiting each individual write operation in a loop.
