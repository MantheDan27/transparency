...
## 2024-08-27 - Batching Firestore Operations
**Learning:** In Firebase/Firestore applications, sequential `await setDoc` calls within loops (like processing large arrays of devices) cause severe N+1 network requests, leading to UI blocking and excessive sync times.
**Action:** Always utilize `writeBatch()` for multiple distinct write operations, ensuring to chunk commits every 500 operations to stay within Firestore limits. This pattern dramatically improves data ingestion performance.
