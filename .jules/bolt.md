...
## 2024-05-29 - [Single O(N) pass for aggregate counts]
**Learning:** Chaining multiple `.filter().length` calls on large arrays to calculate multiple different metrics causes redundant O(N) passes, scaling poorly as array sizes grow and leading to UI rendering lag.
**Action:** Replace multiple sequential O(N) iteration passes with a single O(N) loop that computes all required aggregate counters simultaneously.
