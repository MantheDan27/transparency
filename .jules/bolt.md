...
## 2026-08-10 - Firestore N+1 Batch Ingestion
**Learning:** Sequential `await setDoc` calls inside loops for bulk data ingestion in Firestore cause severe N+1 network request bottlenecks and block the main thread.
**Action:** Always utilize `writeBatch()` to group up to 500 operations per batch when ingesting large datasets to optimize API performance and reduce UI blocking.
