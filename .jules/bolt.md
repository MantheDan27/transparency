...
## 2024-05-19 - Consolidated array iteration
**Learning:** In frontend applications, chained or multiple `.filter().length` calls on large arrays for calculating UI statistics can cause redundant O(N) passes and intermediate array allocations.
**Action:** Consolidate multiple filter passes over the same array into a single `for...of` loop with accumulator variables to improve performance.
