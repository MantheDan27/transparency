...
## 2026-08-08 - Firestore N+1 Query Optimization
**Learning:** Sequential `await setDoc` calls in loops for syncing large JSON payloads block the UI and create significant network overhead due to N+1 API requests.
**Action:** Always utilize Firestore's `writeBatch` when syncing or ingesting arrays of objects, keeping in mind the 500 operations per batch limit.
