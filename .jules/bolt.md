...
## 2024-06-16 - Firestore Bulk Upload Bottlenecks
**Learning:** In the transparency-web frontend client, performing bulk upload and sync operations using sequential `await setDoc` inside a `for...of` loop creates an enormous N+1 network execution bottleneck, as each iteration blocks until the current Firestore write completes. Using `Promise.all` directly allows the client to push the writes concurrently, drastically lowering total operation time.
**Action:** Whenever reviewing bulk operations or loops iterating over network items in a frontend app, replace sequential I/O with an array of promises and `Promise.all()` to prevent execution stalling, being mindful of browser concurrent connection limits.
