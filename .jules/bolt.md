...
## 2025-05-18 - N+1 Network Bottleneck in Firebase
**Learning:** Sequential `await setDoc()` calls within loops create an N+1 network waterfall, severely slowing down bulk operations like importing or syncing devices.
**Action:** Always collect Firestore write promises in an array and execute them concurrently using `Promise.all()` to avoid blocking the loop on each individual network request.
