...
## 2024-05-15 - [Consolidate Array Iterations for UI Statistics]
**Learning:** Chaining multiple `.filter().length` calls on a large array (like `allDevices`) for calculating UI statistics triggers redundant O(N) passes and allocates unnecessary intermediate arrays, creating a rendering bottleneck.
**Action:** When calculating multiple statistics from a single data source, always consolidate iterations into a single `for...of` loop with accumulator variables to reduce complexity from O(K * N) to O(N) and avoid intermediate allocations.
