...
## 2025-08-20 - Batch Firestore writes for bulk data ingestion
**Learning:** Using sequential `await setDoc` calls within loops for large dataset ingestion (like scanning results or desktop sync) causes N+1 network requests, massively degrading performance and potentially blocking the UI thread.
**Action:** Utilize Firestore's `writeBatch()` to group up to 500 operations per batch. This minimizes network overhead, resulting in significantly improved API performance and reduced UI blocking during bulk data processing.
