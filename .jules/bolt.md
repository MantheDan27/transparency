...
## 2024-08-18 - Batch Firestore writes to prevent N+1 sequential network requests
**Learning:** Sequential `await setDoc` calls within loops for large dataset ingestion cause N+1 network requests, leading to severe performance bottlenecks and UI blocking.
**Action:** Utilize Firestore `writeBatch()` to group up to 500 operations per batch to significantly improve API performance during data import and sync.
