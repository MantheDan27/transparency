...
## 2025-07-10 - O(N) Optimization for Render Filter Counts
**Learning:** In `device-monitor-desktop/src/renderer.js`, chaining multiple `.filter().length` calls on large arrays like `allDevices` to count occurrences creates redundant O(N) iterations, causing unnecessary UI rendering latency.
**Action:** Replace multiple `.filter().length` passes with a single O(N) pass loop that maintains multiple counters to minimize processing time and improve frontend responsiveness.
