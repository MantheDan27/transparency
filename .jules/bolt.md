...
## 2025-03-01 - Optimize bulk operations and N+1 queries in Firebase
**Learning:** Sequential `await setDoc()` calls inside loops create an N+1 execution bottleneck for Firebase. Also, doing `Array.prototype.find()` inside a loop against another array causes an O(N * M) time complexity.
**Action:** Replace sequential `await setDoc()` with an array of promises executed via `Promise.all()`. Replace O(N) array lookups inside loops with O(1) lookups using a Map pre-computed outside the loop.
