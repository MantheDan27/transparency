...
## 2025-05-18 - [O(1) Pass Optimization for Device Filters]
**Learning:** Using multiple `.filter().length` calls on a large array (like `allDevices`) creates redundant O(N) iterations, leading to unnecessary main thread blocking and slower UI rendering.
**Action:** Replace multiple filter-and-count operations with a single O(N) loop containing multiple counters. This drastically reduces array iterations and improves UI responsiveness.
