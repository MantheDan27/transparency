...
## 2026-07-28 - [Optimize redundant array filtering iterations in UI]
**Learning:** Chaining multiple `.filter().length` calls on large arrays (like `allDevices`) for computing UI counts causes redundant $O(N)$ passes over the data, which degrades rendering performance.
**Action:** Replace multiple separate $O(N)$ array filter passes with a single $O(N)$ pass loop that increments multiple distinct counters simultaneously to minimize UI rendering latency.
