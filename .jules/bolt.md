...
## 2026-08-15 - Firestore Ingestion Optimization
**Learning:** Using sequential `await setDoc` calls within loops for large dataset ingestion (like device uploads or API syncs) causes an N+1 problem of synchronous network requests, which drastically degrades performance and blocks the main thread.
**Action:** Use `writeBatch(db)` to group up to 500 set operations per batch to significantly improve Firestore API performance and reduce UI blocking.
