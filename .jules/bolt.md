...
## 2025-03-28 - Optimize array searches inside mapping/rendering loops
**Learning:** Using $O(N)$ operations like `Array.prototype.some()`, `find()`, or `includes()` inside mapping or rendering loops (e.g., `forEach` or `map`) creates an $O(N \times M)$ performance anti-pattern.
**Action:** Always pre-compute these lookups into an $O(1)$ lookup structure, such as a `Set` or a `Map`, before the loop begins. Ensure initialization of these structures is fully hoisted outside the mapping loop to avoid re-computation.
