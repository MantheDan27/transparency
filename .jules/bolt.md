...
## 2026-08-11 - Avoid Sequential setDoc for Firestore Ingestion
**Learning:** In Firebase/Firestore applications, using sequential `await setDoc` calls within loops for large dataset ingestion causes N+1 network requests and significant UI blocking.
**Action:** Always utilize `writeBatch()` to group up to 500 operations per batch when ingesting arrays of objects into Firestore to improve API performance and reduce latency.
