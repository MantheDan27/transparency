...
## 2024-07-17 - Avoid multiple filter-length calls on large arrays
**Learning:** In frontend UI code (like `device-monitor-desktop/src/renderer.js`), chaining multiple `.filter().length` calls on a large array (e.g., `allDevices`) for deriving counts creates redundant O(N) iterations, blocking the main thread and increasing UI rendering latency.
**Action:** Use a single O(N) pass loop with multiple counters to calculate all required statistics simultaneously.
