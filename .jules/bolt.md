...
## 2024-06-12 - Resolve N+1 Execution Bottlenecks in Firestore Bulk Operations
**Learning:** Sequential `await setDoc()` calls inside loops during frontend bulk operations (like uploading or syncing devices) create severe N+1 execution bottlenecks, blocking the main thread or delaying UI responsiveness.
**Action:** Replace sequential `await setDoc()` calls inside loops with an array of promises and execute them concurrently via `Promise.all()` to dramatically improve execution speed.
