...
## 2024-06-15 - N+1 Firestore Sync Bottleneck
**Learning:** Sequential `await setDoc()` calls inside bulk device loops (like sync or upload) create massive N+1 bottlenecks.
**Action:** Collect operations in an array and use `Promise.all()` to batch execution and prevent blocking.
