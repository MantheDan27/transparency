...
## 2024-05-24 - Avoid chained filter().length calls
**Learning:** The codebase contains a pattern of calculating multiple UI statistics by repeatedly calling `.filter().length` on large arrays like `allDevices`. This results in redundant $O(N)$ passes and intermediate array allocations that can block the main thread.
**Action:** Always consolidate multiple filter conditions into a single `for...of` loop with accumulator variables when calculating multiple statistics over the same large array.
