...
## 2023-11-20 - [O(1) Statistics Loop vs O(N) Chained Filters]
**Learning:** Chaining multiple `.filter().length` calls on a large array (like `allDevices`) for calculating UI statistics triggers redundant $O(N)$ passes and creates intermediate array allocations, slowing down rendering in the frontend.
**Action:** When computing multiple counts or metrics from a large dataset, consolidate the iterations into a single `for...of` loop using accumulator variables to compute all metrics in one $O(N)$ pass without creating intermediate arrays.
