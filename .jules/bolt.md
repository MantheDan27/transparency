...
## 2025-05-18 - Eliminate Redundant Array Passes in UI Metrics
**Learning:** The codebase previously calculated multiple UI filter counts by chaining multiple `.filter(condition).length` calls on the `allDevices` array, causing redundant O(N) passes and intermediate memory allocations that slow down the main thread.
**Action:** Always consolidate multiple filter counts into a single `for...of` loop with accumulator variables. This reduces O(K*N) complexity to O(N) and eliminates garbage collection overhead from intermediate arrays.
