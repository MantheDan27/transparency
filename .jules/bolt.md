...
## 2024-07-24 - Single Pass O(N) Counter for Large UI Arrays
**Learning:** Chaining multiple `.filter().length` calls on large arrays (like `allDevices`) creates redundant $O(N)$ iterations, leading to $O(K \times N)$ time complexity and causing UI rendering latency during updates.
**Action:** When calculating multiple categorical counts from a large data array in the frontend, replace sequential `.filter()` calls with a single $O(N)$ pass loop utilizing multiple counters.
