...
## 2025-05-18 - [O(N) Optimization for Multiple Array Filter Passes]
**Learning:** Chaining `.filter().length` calls on a large array (like `allDevices`) multiple times to calculate separate UI metrics creates significant performance overhead by forcing O(K * N) iterations and redundantly allocating K intermediate arrays just to check their size.
**Action:** When calculating multiple aggregate counts from a single large array, consolidate all filtering logic into a single `for...of` loop with counter variables. This reduces time complexity to strictly O(N) and eliminates the intermediate array memory footprint.
