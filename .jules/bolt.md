...
## 2026-04-30 - [O(1) Consolidation of Multiple O(N) Array Passes]
**Learning:** Chaining multiple `.filter().length` calls on large arrays inside UI calculation functions creates severe performance bottlenecks by imposing repeated O(N) passes and allocating multiple intermediate, throw-away arrays.
**Action:** Consolidate multiple independent array iteration passes into a single `for...of` loop with scalar accumulator variables to reduce both time complexity ($O(M \times N) \to O(N)$) and memory pressure (eliminating intermediate array allocations).
