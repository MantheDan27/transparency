...
## 2026-08-09 - Firestore N+1 Network Requests
**Learning:** Sequential `await setDoc` calls within loops for large dataset ingestion cause N+1 network requests, severely impacting performance and blocking the UI.
**Action:** Always utilize `writeBatch()` to group up to 500 operations per batch when writing multiple documents in Firestore.
