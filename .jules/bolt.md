...
## 2025-06-01 - Optimize Firestore Bulk Operations
**Learning:** For Firestore bulk operations in frontend clients (e.g., uploading or syncing devices), replacing sequential `await setDoc()` calls inside loops with an array of promises executed via `Promise.all()` significantly resolves N+1 execution bottlenecks. Also, using an $O(N)$ operation inside a loop over devices creates a massive performance bottleneck ($O(N \times M)$ complexity).
**Action:** When implementing bulk operations, pre-compute a lookup table (e.g. `Map` or `Set`) before the loop and replace sequential `await setDoc()` calls with an array of promises executed concurrently via `Promise.all()`.
