...
## 2025-05-24 - [O(1) Loop Consolidation for Filter Counts]
**Learning:** In frontend applications, chained `.filter().length` calls on large arrays (like `allDevices`) for calculating multiple UI statistics cause redundant O(N) passes and intermediate array allocations that pressure the garbage collector and block the main thread.
**Action:** Consolidate multiple array filtering passes into a single `for...of` loop with accumulator variables to avoid redundant O(N) passes and intermediate array allocations.
