...
## 2024-06-04 - [Firestore Bulk Operations Optimization]
**Learning:** Sequential `await setDoc()` calls inside loops in frontend clients (e.g., syncing devices) create an N+1 execution bottleneck due to repeated network round-trips for each document. Additionally, using `Array.prototype.find()` inside these loops creates an $O(N \times M)$ processing bottleneck.
**Action:** Always replace sequential `await setDoc()` inside loops with an array of promises executed via `Promise.all()`. Pre-compute $O(1)$ lookup tables (`Map`) outside the loop to replace $O(N)$ array searches, ensuring optimal performance for bulk data operations.
