...
## 2024-10-24 - Single Pass Array Iteration
**Learning:** In device-monitor-desktop/src/renderer.js, chaining multiple `.filter().length` calls on large arrays like `allDevices` to count occurrences creates redundant O(N) iterations, causing UI rendering latency.
**Action:** Use a single O(N) pass loop with multiple counters to minimize UI rendering latency instead of using multiple `.filter().length` statements.
