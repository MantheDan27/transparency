...
## 2026-10-25 - [O(1) Loop Optimization for Array Stats]
**Learning:** Using multiple, chained `$O(N)$` operations (like `.filter().length`) to compute various statistics over a large array (like `allDevices`) creates a significant performance bottleneck ($O(K \times N)$ complexity, where $K$ is the number of filters) and unnecessarily allocates intermediate arrays in memory.
**Action:** When calculating multiple statistics from a single large dataset, consolidate the iterations into a single `for...of` loop with accumulator variables. This reduces time complexity to a strict $O(N)$ and eliminates intermediate memory allocation overhead.
