...
## 2023-10-27 - Firestore Bulk Write N+1 Bottleneck
**Learning:** In Firebase/Firestore applications (like `transparency-web`), sequential `await setDoc` calls within loops for large dataset ingestion cause N+1 network requests, severely impacting performance and blocking the UI.
**Action:** Utilize Firestore's `writeBatch()` to group up to 500 operations per batch to significantly improve API performance, turning O(N) network requests into O(N/500).
