...
## 2025-03-23 - [O(1) Array Filter Optimization]
**Learning:** Using chained or repeated `.filter().length` calls on large arrays (e.g., `allDevices`) to calculate multiple UI statistics creates redundant $O(N)$ passes and intermediate array allocations, leading to performance bottlenecks when rendering updates.
**Action:** Consolidate multiple statistics calculations into a single `for...of` loop with accumulator variables to reduce the overhead from $O(N \times M)$ passes to a single $O(N)$ pass, minimizing garbage collection pressure.
