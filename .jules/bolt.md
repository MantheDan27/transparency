...
## 2024-05-30 - O(N) Iteration Consolidation
**Learning:** Using chained `.filter().length` calls on large arrays (like `allDevices`) for computing multiple UI statistics triggers redundant O(N) passes and unnecessary intermediate array allocations, resulting in slow rendering performance on large datasets.
**Action:** Consolidate multiple iteration-based metrics into a single `for...of` loop with accumulator variables. This changes O(M * N) complexity to O(N) and eliminates intermediate array allocations.
