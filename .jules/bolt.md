...
## 2024-06-28 - Optimize Firestore Writes
**Learning:** Sequential `await setDoc()` calls inside loops cause serious performance degradation (network waterfalls) when bulk syncing or importing many records to Firestore.
**Action:** Use `Promise.all()` to execute network writes in parallel when ordering doesn't strictly matter.
