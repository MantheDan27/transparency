...
## 2024-05-20 - Consolidating redundant array iterations in UI stat counters
**Learning:** Found an anti-pattern in `device-monitor-desktop` frontend logic where chained `.filter().length` calls were used repeatedly on the main `allDevices` array to calculate UI statistic summary counts. This resulted in redundant array allocations and multiple $O(N)$ iteration passes over the same dataset.
**Action:** When updating UI counts or aggregating metrics based on multiple conditions of a single large array, always consolidate the conditions into a single `for...of` loop with multiple accumulator variables to reduce overhead from $O(k \cdot N)$ to $O(N)$ and avoid intermediate array creation.
