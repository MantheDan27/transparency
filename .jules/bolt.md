...
## 2025-03-24 - [O(1) Network Call Optimization via writeBatch]
**Learning:** Iterating over large datasets and calling Firebase `setDoc` sequentially causes an N+1 network request pattern, blocking the UI and drastically slowing down data ingestion from device scans.
**Action:** When implementing bulk data ingestion logic with Firebase/Firestore, utilize `writeBatch()` to group up to 500 operations per batch. This reduces API overhead and significantly improves performance for large data sets compared to sequential writes.
