...
## 2024-05-24 - N+1 Asynchronous loop bottleneck
**Learning:** Using `await` sequentially inside a loop (like iterating through devices to write `setDoc`) creates a significant N+1 performance bottleneck that drastically slows down bulk updates and uploads.
**Action:** Always refactor sequential asynchronous loops to collect operations in an array and use `Promise.all(promises)` to execute them concurrently.
