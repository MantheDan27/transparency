...
## 2023-10-27 - Firestore N+1 Performance Bottleneck Solved with Batching
**Learning:** Sequential `await setDoc` operations inside large loops (e.g., bulk device ingestion in `transparency-web`) block the main thread and generate massive, individual network requests, severely impacting application performance and UI responsiveness.
**Action:** Always utilize `writeBatch()` for multiple Firestore writes. Group operations in batches (up to 500 per Firestore limits) and periodically `batch.commit()` to aggregate network overhead and vastly improve throughput. Ensure proper modulo logic correctly processes the remainder at the end of the loop.
