...
## 2026-07-26 - Avoid multiple filter length calls on large arrays
**Learning:** In device-monitor-desktop, chaining multiple `.filter().length` calls on large arrays like `allDevices` creates redundant O(N) iterations, causing unnecessary UI rendering latency.
**Action:** Use a single O(N) pass loop with multiple counters to efficiently calculate occurrences without redundant array iterations.
