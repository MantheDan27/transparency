...
## $(date +%Y-%m-%d) - Array processing in UI stats
**Learning:** Chaining multiple `.filter().length` calls on a shared array to compute distinct statistics is a common O(k*N) anti-pattern in JS that creates unnecessary intermediate arrays and CPU overhead, especially problematic in UI rendering paths dealing with large lists (like device inventories).
**Action:** Always replace multiple sequential array filters used solely for counting with a single O(N) pass loop and independent counter variables to minimize memory pressure and UI latency.
