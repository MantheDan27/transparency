...
## 2024-06-29 - Optimize bulk Firebase writes using Promise.all()
**Learning:** In Firebase applications like `transparency-web`, sequential `await setDoc()` calls within a loop create N+1 network waterfalls, severely impacting performance for bulk data operations.
**Action:** Use `Promise.all()` to execute the write promises concurrently to significantly improve performance for bulk operations.
