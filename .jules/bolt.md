...
## 2026-06-09 - N+1 and O(N) Lookup Optimization in Firebase Sync
**Learning:** Using sequential `await setDoc()` inside a device array loop creates massive N+1 network latency bottlenecks during bulk operations. Additionally, using `Array.prototype.find()` inside that same loop to associate subnetworks creates an $O(N \times M)$ processing freeze, significantly delaying large dataset synchronizations.
**Action:** Always replace sequential `await setDoc()` loops with a concurrent array of promises resolved via `Promise.all()`. Pre-compute a lookup `Map` for relational data outside the loop to ensure $O(1)$ constant-time matching during large iterations.
