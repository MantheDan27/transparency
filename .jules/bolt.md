...
## 2024-05-18 - Consolidating redundant array iterations in UI updates
**Learning:** Frontend applications calculating multiple UI statistics from a single large array often use chained `.filter().length` calls. This results in multiple O(N) passes and redundant intermediate array allocations, which degrades UI rendering performance on large datasets.
**Action:** Replace multiple `.filter().length` chains with a single `for...of` loop using accumulator variables to reduce overhead to O(N) and eliminate intermediate memory allocations.
