...
## 2024-05-18 - Consolidating redundant Array.filter calls
**Learning:** Multiple sequential `.filter()` operations on the same large array (like `allDevices`) for calculating discrete UI statistics cause redundant O(N) traversals and intermediate array allocations.
**Action:** Consolidate these multiple passes into a single `for...of` loop with simple accumulator variables to reduce overhead from O(M*N) to O(N).
