...
## 2025-03-24 - [Firestore N+1 Request Bottleneck]
**Learning:** Iterating over large datasets and calling `await setDoc()` inside the loop creates an N+1 API request bottleneck, significantly increasing latency, risking rate limits, and hanging the UI during large sync operations in Firebase web apps.
**Action:** Always utilize Firestore's `writeBatch()` for bulk data insertion. Group operations up to the 500 document limit, then commit and start a new batch. This transforms O(N) network requests into O(N/500).
