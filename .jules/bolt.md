...
## 2025-05-15 - [Optimize Firestore Bulk Operations]
**Learning:** Sequential `await setDoc()` calls inside a loop during bulk document inserts create an N+1 execution bottleneck, causing significant UI freezing or unresponsive behaviour while large numbers of network calls are made sequentially.
**Action:** Always batch related Firebase operations using an array of promises executed via `await Promise.all()` to fire network requests concurrently, improving performance for bulk operations significantly.
