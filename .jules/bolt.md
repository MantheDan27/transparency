...
## 2026-07-16 - Optimize multi-filter counting

**Learning:** Chaining multiple `.filter().length` calls on a large array (like `allDevices`) creates redundant O(N) iterations, leading to unnecessary UI rendering latency.
**Action:** Consolidate multiple filter counts into a single O(N) pass loop using multiple counters to minimize iteration overhead.
