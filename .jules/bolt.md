...
## 2024-05-18 - Optimized Array Iteration in UI Update
**Learning:** Avoid using multiple `Array.prototype.filter().length` calls sequentially when calculating statistics or UI counts over the same dataset. This results in O(N * C) complexity where C is the number of filters.
**Action:** When computing multiple different counts or categorizations from a single array, consolidate them into a single O(N) iteration loop that tallies all metrics simultaneously.
