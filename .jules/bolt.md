...
## 2024-05-23 - Consolidate chained filter passes for UI rendering
**Learning:** UI statistic calculations like filter counts often lead to chained `.filter().length` calls on the same large array collection (e.g. `allDevices`). This executes $O(N)$ operations repeatedly, causing unnecessary main-thread blocking when rendering tables or maps.
**Action:** Always replace multiple chained `.filter().length` checks on the same array with a single `for...of` loop and accumulator variables.
