...
## 2024-06-06 - Firebase Batch Writing & Map Lookups
**Learning:** Resolving N+1 bottlenecks in Firestore is crucial when bulk uploading devices. Sequential `await setDoc()` calls severely block the event loop. Combining with O(N) array lookups (e.g. `Array.prototype.find()`) nested within the bulk operation compounds this into O(N*M) delay.
**Action:** Replaced sequential `await` loops with arrays of promises executing via `Promise.all()`. Converted inner-loop linear lookups into pre-computed O(1) `Map` operations outside the loop to drastically reduce processing overhead during sync.
