...
## 2024-05-23 - Avoid multiple .filter().length calls on large arrays
**Learning:** Chaining multiple `.filter().length` calls on large arrays (like `allDevices`) for counting occurrences creates redundant O(N) iterations, leading to UI rendering latency.
**Action:** Use a single O(N) pass loop with multiple counters to minimize UI rendering latency when computing metrics for various categories.
