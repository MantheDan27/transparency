...
## 2024-05-17 - Consolidate Array Passes for UI Statistics
**Learning:** Chaining `.filter().length` on large arrays (like `allDevices`) for calculating multiple UI statistics creates redundant $O(N)$ passes and intermediate array allocations, significantly degrading rendering performance.
**Action:** Consolidate multiple statistics calculations into a single `for...of` loop over the array, using accumulator variables. This reduces time complexity from $O(K \times N)$ to $O(N)$ and eliminates garbage collection overhead from intermediate arrays.
