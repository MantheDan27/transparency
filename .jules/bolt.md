...
## 2026-07-11 - Optimize multiple array iterations
**Learning:** In device-monitor-desktop, chaining multiple `.filter().length` calls on large arrays like `allDevices` to count occurrences creates redundant O(N) iterations. This degrades performance when the array is large.
**Action:** Instead, use a single O(N) pass loop with multiple counters to minimize UI rendering latency and reduce redundant iterations.
