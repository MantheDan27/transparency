...
## 2025-05-31 - [N+1 Bottleneck in Frontend Firestore Sync]
...
**Action:** When performing bulk writes to a cloud database from the frontend, batch them or execute them concurrently via `Promise.all()` instead of awaiting them sequentially in a loop.
