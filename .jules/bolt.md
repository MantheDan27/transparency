...
## 2026-08-22 - Batch Firestore Writes to Prevent N+1 Requests
**Learning:** Sequential `await setDoc` operations in a loop for large data imports create an N+1 request pattern that bottlenecks performance.
**Action:** Always use Firestore's `writeBatch` (grouping up to 500 operations per commit) for bulk data ingestion to significantly reduce API calls and prevent UI freezing.
