...
## 2026-08-02 - Avoid Multiple Filters on allDevices
**Learning:** In the device monitor UI, computing filter counts using multiple chained `.filter().length` on large arrays like `allDevices` creates redundant O(N) iterations that block the main thread and cause UI rendering latency.
**Action:** Replace multiple O(N) `.filter().length` iterations with a single O(N) pass loop that maintains multiple counters simultaneously to minimize latency.
