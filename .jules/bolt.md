...
## 2024-06-08 - Optimize Firestore Bulk Operations
**Learning:** Sequential `await setDoc()` calls inside loops create a significant N+1 execution bottleneck for bulk Firestore operations (e.g. uploading or syncing devices).
**Action:** Always replace sequential `await setDoc()` calls with an array of promises executed via `Promise.all()` to concurrently process bulk operations.
