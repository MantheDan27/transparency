...
## 2024-08-01 - Avoid chaining multiple filter().length calls
**Learning:** Found an anti-pattern where multiple `.filter(...).length` were used to count occurrences of different conditions on a large array (`allDevices`), leading to redundant O(N) loops.
**Action:** Always combine multiple `.filter(...).length` counting operations into a single O(N) loop with multiple counters to optimize rendering latency.
