...
## 2026-07-27 - Single Pass Loop Over allDevices
**Learning:** In device-monitor-desktop, chaining multiple `.filter().length` calls on large arrays like `allDevices` to count occurrences creates redundant O(N) iterations, causing UI rendering latency.
**Action:** Use a single O(N) pass loop with multiple counters to minimize UI latency.
