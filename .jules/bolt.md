...
## 2024-06-17 - Firebase Web App N+1 Executions
**Learning:** Sequential `await setDoc()` inside loops causing major N+1 execution bottlenecks.
**Action:** Always replace sequential `await setDoc()` with an array of promises executed via `Promise.all()` to resolve N+1 execution bottlenecks.
