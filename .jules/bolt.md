...
## 2024-07-07 - Optimize Array Iterations in UI Filters
**Learning:** Chaining multiple `.filter().length` calls on a large array (`allDevices`) within a UI update function creates redundant O(N) iterations, causing unnecessary UI rendering latency.
**Action:** Replace multiple filtering passes with a single O(N) loop containing multiple conditional counters to minimize processing time and improve UI responsiveness.
