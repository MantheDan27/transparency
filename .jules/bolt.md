...
## 2025-05-25 - [O(1) Pass Consolidation for Array Filtering]
**Learning:** Using chained `.filter().length` calls on large arrays (like `allDevices`) for calculating multiple UI statistics creates redundant $O(N)$ passes and intermediate array allocations.
**Action:** Consolidate these into a single `for...of` loop with accumulator variables to avoid redundant passes and allocations.
