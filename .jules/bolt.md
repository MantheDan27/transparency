...
## 2024-05-31 - [Single Pass Array Iteration for UI Rendering]
**Learning:** In device-monitor-desktop/src/renderer.js, chaining multiple `.filter().length` calls on large arrays like `allDevices` to count occurrences creates redundant O(N) iterations and causes UI rendering latency.
**Action:** Use a single O(N) pass loop with multiple counters to minimize UI rendering latency when processing large lists of devices.
