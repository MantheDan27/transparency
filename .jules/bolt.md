...
## 2026-08-17 - Batch Firestore Writes for N+1 Problem
**Learning:** Sequential \`await setDoc\` calls within loops cause N+1 network requests in Firebase, severely impacting performance for data ingestion.
**Action:** Utilize Firestore's \`writeBatch()\` to group operations (up to 500 per batch) to significantly improve API performance and reduce UI blocking.
