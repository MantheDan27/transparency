...
## 2026-08-03 - [O(N) Filter Chaining Bottleneck in UI Counters]
**Learning:** Chaining multiple `.filter().length` calls on large arrays (like `allDevices` in `device-monitor-desktop/src/renderer.js`) to count occurrences creates redundant O(N) iterations, causing UI rendering latency during state updates.
**Action:** Replace multiple `.filter().length` operations with a single O(N) pass loop and multiple counter variables when updating dashboard statistics.
