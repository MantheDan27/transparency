...
## 2024-05-28 - Add autocomplete and disabled states to auth forms
**Learning:** In vanilla HTML/JS applications without complex form state libraries, native `<input autocomplete="...">` attributes are critical for password managers to reliably assist users. Additionally, explicit disabled states during async submissions prevent double-submission errors and improve perceived performance.
**Action:** Always include appropriate `autocomplete` attributes on auth forms (`email`, `current-password`, `new-password`) and enforce UI disabled states on submit buttons during `await` calls.
