...
## 2024-05-15 - Missing disabled CSS rules for primary buttons
**Learning:** In this design system, `btn-primary` lacks CSS `:disabled` rules, meaning disabled buttons don't provide visual feedback by default.
**Action:** When implementing async form submissions, use JavaScript inline styling (`opacity: "0.7"`, `cursor: "not-allowed"`) alongside the native `disabled` attribute, and ensure they are cleared using empty strings in a `finally` block to restore the original state correctly.
