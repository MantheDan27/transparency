...
## 2024-05-26 - Async Form Submission Loading States

**Learning:** When dealing with asynchronous form submissions (like authentication forms) in single-page applications, it is a common pattern to omit visual feedback during the network request. This can lead to users clicking the submit button multiple times, causing errors or confusion. Furthermore, missing `autocomplete` attributes on sensitive fields like emails and passwords frustrates users relying on password managers.

**Action:** Proactively add explicit loading/disabled states to submission buttons during async requests, ensuring they are restored in a `finally` block to prevent them from becoming permanently stuck. Additionally, always inject appropriate `autocomplete` attributes (`email`, `current-password`, `new-password`, `name`) into authentication forms to aid usability.
