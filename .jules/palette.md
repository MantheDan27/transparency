...
## 2026-09-05 - Contextual ARIA labels on dynamic card actions
**Learning:** When generating interactive cards containing identical generic buttons (e.g., "View Devices", "Remove"), screen readers announce these buttons out-of-context. Since these are visually associated with a specific card but lack programmatic context, they require contextual identification.
**Action:** Always inject `aria-label` attributes on dynamically generated card action buttons, dynamically populating them with identifying data (like the card title) to ensure screen reader users can distinguish between them.
