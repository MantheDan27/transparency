...
## 2024-05-26 - Disabled Button Styles Constraint in Transparency Web
**Learning:** The `transparency-web` application's `public/css/style.css` file lacks explicit `:disabled` pseudo-class styling for primary buttons like `.btn-primary`. Standard HTML `disabled` attributes alone fail to provide necessary visual feedback to users during asynchronous form submission.
**Action:** When implementing disabled or loading states for asynchronous operations under the strict 'no custom CSS' constraint in this repository, use JavaScript inline styling (`style.opacity = "0.7"`, `style.cursor = "not-allowed"`) combined with the native `disabled` attribute, and always ensure styles are cleared (`style.opacity = ""`) in a `finally` block to restore original states upon completion or error without overriding existing CSS classes.
