...
## 2024-05-26 - Inline Styling for Disabled UI States

**Learning:** In projects like `transparency-web` where the primary stylesheet (`style.css`) lacks `:disabled` pseudo-class definitions for buttons, simply relying on the native `disabled` attribute will not visually communicate the disabled state. This design constraint means that without additional styling, users may be confused if an async request is in flight.

**Action:** When adding disabled states to buttons in such restricted environments, use JavaScript inline styling (e.g., `element.style.opacity = "0.7"; element.style.cursor = "not-allowed"`) to explicitly enforce the visual cue, remembering to revert these inline styles to `""` (empty string) afterward to restore the element's natural class-driven appearance.
