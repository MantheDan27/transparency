...
## 2026-07-18 - Missing disabled CSS states in Transparency Web
**Learning:** The `transparency-web` design system (via `public/css/style.css`) lacks `:disabled` rules for `.btn-primary` and other buttons. Because we have a "no custom CSS" constraint, we can't add standard CSS pseudo-classes.
**Action:** When implementing disabled or loading states for asynchronous actions, use JavaScript inline styling (`opacity = "0.7"`, `cursor = "not-allowed"`) alongside the native `disabled` attribute. Always restore these to empty strings `""` in a `finally` block to preserve original hover states.
