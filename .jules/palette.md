...
## 2024-05-26 - Missing :disabled CSS rules and Inline Styling Constraints
**Learning:** In `transparency-web`, the design system (`public/css/style.css`) lacks `:disabled` state rules for primary action buttons (like `.btn-primary`). Because of the constraint against adding custom CSS, standard `disabled=true` attributes do not provide visual feedback.
**Action:** When implementing disabled or loading states for asynchronous form submissions in this app, combine the native `disabled` attribute with temporary JavaScript inline styling (`element.style.opacity = "0.7"`; `element.style.cursor = "not-allowed"`) to convey the state visually. Ensure these inline styles are cleared with an empty string (`""`) in a `finally` block to prevent overriding existing CSS hover states when the button is re-enabled.
