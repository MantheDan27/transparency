...
## 2026-07-06 - Loading States on Async Forms
**Learning:** In vanilla JS apps, form submission buttons often lack explicit loading states during async operations, leading to users repeatedly clicking the button and triggering multiple concurrent requests.
**Action:** Always add loading states (e.g., `submitBtn.disabled = true`, changing text, reducing opacity) to submit buttons during async operations, and critically, ensure they are reset using a `finally` block to prevent the UI from getting permanently stuck on error.
