...
## 2024-07-26 - Missing :disabled CSS rules in transparency-web
**Learning:** The `public/css/style.css` file lacks `:disabled` styles for buttons (like `.btn-primary`).
**Action:** When implementing disabled or loading states, use JavaScript inline styling (`opacity = "0.7"`, `cursor = "not-allowed"`) along with the native `disabled` attribute, and ensure to clear the inline styles (`opacity = ""`) in a `finally` block to prevent breaking hover states when re-enabled.
