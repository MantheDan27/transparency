...
## 2024-05-18 - Missing Disabled/Loading States
**Learning:** Found multiple asynchronous forms and interactive buttons across `auth.js` and `dashboard.js` without loading states or disabled functionality during API calls, creating poor user feedback.
**Action:** Always add loading and disabled states to buttons making async calls, wrapping the resetting of state within `finally` blocks to ensure the buttons are re-enabled if errors occur.
