...
## 2026-07-10 - Handling loading states for asynchronous form submissions
**Learning:** Adding loading states (like disabling buttons and changing text) is important for UX, but must strictly account for error conditions. Modifying standard form UI components before network requests require restoring the initial component states properly on failure.
**Action:** Always wrap the actual submit and subsequent resetting of UI form elements like button disabled, opacity, and text properties in a standard `try...finally` block.
