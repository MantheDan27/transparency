...
## 2024-07-04 - Accessible Decorative SVGs vs Functional Canvases
**Learning:** When retrofitting accessibility attributes (`aria-hidden="true"`) to decorative SVGs across an app, it's critical to explicitly exclude SVGs that serve as functional visualization canvases (like D3.js maps) because they carry semantic or interactive weight, unlike pure icons.
**Action:** Always verify if an `<svg>` tag is used merely for iconography or as an interactive canvas before hiding it from screen readers. Use specific IDs or classes (e.g., `#networkMap`) as exclusion filters when automating accessibility attribute additions.
