...
## 2024-07-05 - Auth form loading states
**Learning:** Adding loading states to authentication forms (Login and Signup) is a crucial UX improvement that provides immediate visual feedback, prevents duplicate submissions during network operations, and improves accessibility by disabling the button while processing.
**Action:** Always implement a `try...finally` block for async form submissions to ensure the button state (text, disabled attribute, opacity, cursor) is consistently updated and restored regardless of success or failure.
