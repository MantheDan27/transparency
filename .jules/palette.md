...
## 2026-06-09 - Scoping Variables in Vanilla JS Form Handlers
**Learning:** When refactoring multiple event listeners in vanilla JavaScript (like adding loading states to login and signup forms), applying structural changes across the file can inadvertently inject logic blocks into the wrong scope if not meticulously verified. For instance, attempting to add a `finally` block to reset a `submitBtn` variable inside an unrelated event listener (e.g., `forgotPasswordLink`) where `submitBtn` is undefined will result in a runtime `ReferenceError`, breaking the associated feature.
**Action:** Always strictly verify the full context of every block being modified. Use precise string replacements or context-aware AST manipulation, and never assume structural similarity across disparate event listeners without inspecting the exact variable definitions in scope.
