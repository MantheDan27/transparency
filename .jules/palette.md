...
## 2024-05-24 - Missing Loading States on Authentication Forms
**Learning:** In vanilla JS applications lacking dedicated component libraries, async operations like authentication often omit explicit button loading states, leaving users without feedback and susceptible to multiple submissions.
**Action:** Always verify that form submit handlers in vanilla JS explicitly disable the submission button and provide a visual loading indicator (e.g., text change) during the `try/catch` execution of asynchronous API calls.
