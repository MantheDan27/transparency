...
## 2024-06-28 - Fallback Styling for Missing Disabled CSS Rules
**Learning:** When adding accessibility or UX enhancements like disabled button states to a codebase without explicit `:disabled` CSS rules for that component, relying solely on the HTML `disabled` attribute can leave the UI visually unchanged, confusing users.
**Action:** When a `.btn-primary:disabled` rule is absent and you cannot add custom CSS per boundaries, use inline JavaScript styling (e.g., `submitBtn.style.opacity = "0.7"; submitBtn.style.cursor = "not-allowed";`) in conjunction with the `disabled` attribute to provide immediate visual feedback, ensuring you revert the styles in a `finally` block to prevent permanent UI locks on error.
