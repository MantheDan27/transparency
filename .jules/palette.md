...
## 2025-01-20 - Inline JS for Missing CSS Disabled States
**Learning:** In transparency-web, the public/css/style.css file lacks `:disabled` rules for buttons like `.btn-primary`. Standard HTML `disabled` attributes do not provide visual feedback, creating confusing async loading states.
**Action:** To implement disabled UI states while adhering to the 'no custom CSS' constraint, use JavaScript inline styling (e.g., `submitBtn.style.opacity = "0.7"`; `submitBtn.style.cursor = "not-allowed"`;) combined with the native disabled attribute. Always clear the inline style (`""`) in a `finally` block to restore original states.
