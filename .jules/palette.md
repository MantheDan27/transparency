...
## 2024-08-03 - Dynamic ARIA attributes on expand/collapse toggles
**Learning:** For dynamic disclosure widgets (like alert guidance), screen readers need to know both what the toggle controls (`aria-controls`) and its current state (`aria-expanded`). Without these, users relying on assistive technology have no context that clicking the button will reveal additional content.
**Action:** When implementing expand/collapse toggles, always link the button to the content container using `aria-controls` and dynamically update the `aria-expanded` state on the button when the content's visibility changes.
