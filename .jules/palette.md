...
## 2025-03-09 - Improve auth form UX with autocomplete and disabled states
**Learning:** Adding `autocomplete` attributes to authentication forms and explicit UI loading/disabled states to submit buttons prevents double-submissions, aids password managers, and improves overall accessibility. This is a critical pattern for all auth flows.
**Action:** Always add autocomplete attributes (e.g., `username`, `current-password`) to login/signup forms, and implement `disabled=true` and loading text on buttons during async requests.
