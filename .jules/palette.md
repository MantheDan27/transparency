...
## 2024-05-26 - Missing :disabled Rules in Vanilla CSS

**Learning:** In transparency-web, the public/css/style.css file lacks `:disabled` rules for buttons like `.btn-primary`. To implement disabled UI states while adhering to the 'no custom CSS' constraint, developers must use JavaScript inline styling.

**Action:** Always use JavaScript inline styling (e.g., `submitBtn.style.opacity = "0.7"; submitBtn.style.cursor = "not-allowed";`) combined with the native `disabled` attribute to implement disabled states, and ensure these properties are reset to empty strings in a `finally` block to prevent UI lock-up.
