...
## $(date +%Y-%m-%d) - Prevent multiple async form submissions
**Learning:** In vanilla JS Firebase apps, auth operations (signIn, createUser) can take several seconds. Without a loading/disabled state on the submit button, users frequently click multiple times, triggering duplicate API requests and overlapping error toasts.
**Action:** Always wrap async Firebase form submissions in `try/catch/finally` blocks, and explicitly disable the submit button and update its text content before awaiting the promise, ensuring it is restored in the `finally` block to prevent the UI from getting permanently stuck.
