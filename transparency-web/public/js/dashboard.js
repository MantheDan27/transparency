...
uploadForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // ⚡ Bolt: Batching Firestore writes to fix N+1 performance issue (~99% fewer requests for large uploads)
    let count = 0;
    let batchCount = 0;
...
syncDevicesBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    // ⚡ Bolt: Batching Firestore writes to fix N+1 performance issue (~99% fewer requests for large syncs)
    let count = 0;
    let batchCount = 0;
    let batch = writeBatch(db);
    for (const dev of devices) {
