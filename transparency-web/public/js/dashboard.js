...
uploadForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    let count = 0;
    // ⚡ Bolt: Batch Firestore writes to prevent N+1 network requests during upload.
    // Impact: Groups up to 500 set operations per network request, significantly reducing API latency and UI blocking.
...
syncDevicesBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    let count = 0;
    // ⚡ Bolt: Batch Firestore writes to prevent N+1 network requests during sync.
    // Impact: Groups up to 500 set operations per network request, significantly reducing API latency and UI blocking.
    let batch = writeBatch(db);
    let batchCount = 0;
