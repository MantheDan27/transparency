...
uploadForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // ⚡ Bolt Optimization: Group setDoc promises to avoid N+1 bottleneck
    let count = 0;
...
syncDevicesBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    // ⚡ Bolt Optimization: Group setDoc promises and use O(1) network lookups
    const syncPromises = [];
    let count = 0;

    const networkSubnetMap = new Map();
    for (const net of allNetworksCache) {
      if (net.subnet) networkSubnetMap.set(net.subnet, net.id);
    }
    ...
    for (const dev of devices) {
      ...
      if (dev.subnet && networkSubnetMap.has(dev.subnet)) {
        networkId = networkSubnetMap.get(dev.subnet);
      }
