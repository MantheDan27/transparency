...
uploadForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // ⚡ Bolt Optimization: Replaced sequential await setDoc with writeBatch
    // Reduces Firestore API roundtrips from O(N) to O(N/500), significantly speeding up bulk uploads.
    let count = 0;
...
syncDevicesBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    // ⚡ Bolt Optimization: Batching desktop sync data to prevent N+1 API calls.
    // Groups up to 500 writes into a single network request to minimize UI blocking.
    let count = 0;
    let batch = writeBatch(db);
    let batchCount = 0;
    for (const dev of devices) {
