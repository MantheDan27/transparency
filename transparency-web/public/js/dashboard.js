...
uploadForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    let count = 0;
    // ⚡ Bolt: Collecting promises instead of sequential await prevents N+1 execution bottleneck
    // Expected impact: Speeds up bulk uploads from O(N) network roundtrips to a single O(1) batch.
    const promises = [];
...
syncDevicesBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    let count = 0;
    // ⚡ Bolt: Collecting promises instead of sequential await prevents N+1 execution bottleneck
    // Expected impact: Speeds up bulk syncs from O(N) network roundtrips to a single O(1) batch.
    const promises = [];
    for (const dev of devices) {
