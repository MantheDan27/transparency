...
uploadForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    let count = 0;
    // ⚡ Bolt: Using writeBatch to prevent N+1 network requests during bulk ingestion
    ...
    for (const dev of devices) {
      ...
      // ⚡ Bolt: Commit batch every 500 operations (Firestore limit)
      if (batchCount === 500) {
        await batch.commit();
        batch = writeBatch(db);
        batchCount = 0;
...
syncDevicesBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    let count = 0;
    // ⚡ Bolt: Using writeBatch to prevent N+1 network requests during bulk sync
    let batch = writeBatch(db);
    let batchCount = 0;
