...
function restartLocalApiWithAuth() {
  ...
  localApiServer = require('http').createServer((req, res) => {
    ...
    if (req.url !== '/' && req.url !== '/api/health') {
      const reqKey = typeof req.headers['x-api-key'] === 'string' ? req.headers['x-api-key'] : '';
      if (apiKeyStore.key) {
        const storedBuf = Buffer.from(apiKeyStore.key);
        const reqBuf = Buffer.from(reqKey);
        const isValid = storedBuf.length === reqBuf.length && require('crypto').timingSafeEqual(storedBuf, reqBuf);
        if (!isValid) {
          res.writeHead(401);
