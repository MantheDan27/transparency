...
function restartLocalApiWithAuth() {
  ...
  localApiServer = require('http').createServer((req, res) => {
    ...
    if (req.url !== '/' && req.url !== '/api/health') {
      ...
      let isMatch = false;
      if (apiKeyStore.key && reqKey) {
        const a = Buffer.from(reqKey);
        const b = Buffer.from(apiKeyStore.key);
        if (a.length === b.length) {
          isMatch = require('crypto').timingSafeEqual(a, b);
        }
      } else if (!apiKeyStore.key) {
        isMatch = true; // allow bypass if key is unconfigured
      }
