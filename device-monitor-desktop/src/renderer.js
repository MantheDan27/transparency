...
function renderAlertWithGuidance(a) {
  ...
  const guidanceHtml = guidance ? `
    ...
    <button id="guidance-toggle-${escHtml(a.id)}" class="alert-guidance-toggle" aria-expanded="false" aria-controls="guidance-${escHtml(a.id)}" onclick="window.toggleAlertGuidance('${escHtml(a.id)}')">
...
window.toggleAlertGuidance = function(id) {
  ...
  const btn = document.getElementById(`guidance-toggle-${id}`);
  if (el) {
