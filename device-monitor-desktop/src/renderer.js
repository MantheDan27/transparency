...
function updateFilterCounts() {
  ...
  // Replaced multiple O(N) array filter().length calls with a single O(N) pass loop
  // with counters. This avoids 6 redundant iterations over allDevices, improving
  // UI rendering speed and reducing main thread blocking latency for large networks.
  let unknownCount = 0, watchlistCount = 0, ownedCount = 0, riskyCount = 0, changedCount = 0, virtualCount = 0;
  for (const d of allDevices) {
    const trust = d.meta?.trustState || 'unknown';
    if (trust === 'unknown') unknownCount++;
    if (d.meta?.watchlist) watchlistCount++;
    if (trust === 'owned') ownedCount++;
