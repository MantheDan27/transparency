...
function updateFilterCounts() {
  ...
  // ⚡ Bolt Performance Optimization: Consolidate multiple O(N) .filter().length passes into a single O(N) pass to reduce array allocations and iteration overhead
  let countUnknown = 0;
  ...
  for (const d of allDevices) {
    ...
    if (trustState === 'unknown') countUnknown++;
    if (d.meta?.watchlist) countWatchlist++;
    if (trustState === 'owned') countOwned++;
