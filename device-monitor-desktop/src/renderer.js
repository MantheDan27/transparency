...
function updateFilterCounts() {
  ...
  // Replace 6x O(N) .filter().length array allocations with a single O(N) pass loop
  // using multiple counters to minimize memory pressure and UI rendering latency.
  let countUnknown = 0, countWatchlist = 0, countOwned = 0, countRisky = 0, countChanged = 0, countVirtual = 0;
  for (const d of allDevices) {
    const trust = d.meta?.trustState || 'unknown';
    if (trust === 'unknown') countUnknown++;
    if (d.meta?.watchlist) countWatchlist++;
    if (trust === 'owned') countOwned++;
